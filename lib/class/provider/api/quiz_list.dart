...
import 'package:sakai_mobile/function/api/get_quiz.dart';

const _PREF_ROOT_KEY = 'siteQuizList';

class QuizListProvider extends StateNotifier<List<Quiz>?> {
  QuizListProvider(super.state);
  ...
  Future<void> init(SharedPreferences pref, String siteId) async {
    ...
    state = QuizList.fromJson(jsonDecode(pref.getString(
  ...
  Future<bool> refresh(SharedPreferences pref, String domain) async {
    ...
    final quizList = await getSiteQuizList(_siteId!, domain);
  ...
  Future<void> _updateStoreApiValue(SharedPreferences pref) async {
    pref.setString('${_PREF_ROOT_KEY}_${_siteId}_${StorePrefKey.API_VALUE}',
        jsonEncode(QuizList(body: state).toJson()));
  }
}
